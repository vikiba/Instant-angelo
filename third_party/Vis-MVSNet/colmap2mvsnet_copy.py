...
if __name__ == '__main__':
    ...
    for image_id in images.keys():
        zs = []
        for p3d_id in images[image_id].point3D_ids:
            if p3d_id == -1:
                continue
            transformed = np.matmul(extrinsic[image_id], [points3d[p3d_id].xyz[0], points3d[p3d_id].xyz[1], points3d[p3d_id].xyz[2], 1])
        ...
        # determine depth number by inverse depth setting, see supplementary material
        if args.max_d == 0:
            image_int = intrinsic[images[image_id].camera_id]
    ...
    def calc_score(inputs):
        i, j = inputs
        id_i = images[image_ids[i]].point3D_ids
        id_j = images[image_ids[j]].point3D_ids
        id_intersect = [it for it in id_i if it in id_j]
        cam_center_i = -np.matmul(extrinsic[image_ids[i]][:3, :3].transpose(), extrinsic[image_ids[i]][:3, 3:4])[:, 0]
        cam_center_j = -np.matmul(extrinsic[image_ids[j]][:3, :3].transpose(), extrinsic[image_ids[j]][:3, 3:4])[:, 0]
        score = 0
    ...
    for i in range(len(images)):
        ...
        view_sel[image_id] = [(image_ids[k], score[i, k]) for k in sorted_score[:10]]
    ...
    for image_id in images.keys():
        if args.convert_format:
            ...
            cv2.imwrite(os.path.join(renamed_dir, '%d.jpg' % int(image_id)-1), img)
        else:
